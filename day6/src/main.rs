...
impl Races {
    fn ways_to_beat_product(&self) -> u64 {
        self.races
            .iter()
            .fold(1, |product, race| product * race.ways_to_beat())
    }
    ...
    fn collate_races(&self) -> Races {
        ...
        let mut new_race = self.races.iter().fold(race, |mut result, race| {
            ...
            result.time = collated_time
                .parse::<u64>()
                .expect("Collate races: invalid time");

            let mut collated_distance = result.distance.to_string();
            collated_distance.push_str(race.distance.to_string().as_str());
            result.distance = collated_distance
                .parse::<u64>()
                .expect("Collate races: invalid distance");
            result
        });
        Races {
            races: vec![new_race],
...
impl From<&Vec<&str>> for Races {
    fn from(lines: &Vec<&str>) -> Self {
        ...
        let distances = lines[1]
            ...
            .expect("Distances not found")
            .split_whitespace();
        let races = times
            .zip(distances)
            .map(|(time, distance)| Race {
                time: time.parse::<u64>().expect("Time expected u64"),
                distance: distance.parse::<u64>().expect("Distance expected u64"),
            })
            .collect();
        Races { races: races }
    }
